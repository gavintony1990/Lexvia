...
func AudioHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	audioAdaptor, ok := adaptor.(channel.AudioAdaptor)
	if !ok {
		return types.NewError(fmt.Errorf("channel does not support audio requests"), types.ErrorCodeConvertRequestFailed, types.ErrOptionWithSkipRetry())
	}
	ioReader, err := audioAdaptor.ConvertAudioRequest(c, info, *request)
	if err != nil {
