...
func RerankHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		rerankAdaptor, ok := adaptor.(channel.RerankAdaptor)
		if !ok {
			return types.NewError(fmt.Errorf("channel does not support reranking"), types.ErrorCodeConvertRequestFailed, types.ErrOptionWithSkipRetry())
		}
		convertedRequest, err := rerankAdaptor.ConvertRerankRequest(c, info.RelayMode, *request)
		if err != nil {
