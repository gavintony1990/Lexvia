...
func ImageHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		imageAdaptor, ok := adaptor.(channel.ImageAdaptor)
		if !ok {
			return types.NewError(fmt.Errorf("channel does not support image generation"), types.ErrorCodeConvertRequestFailed)
		}
		convertedRequest, err := imageAdaptor.ConvertImageRequest(c, info, *request)
		if err != nil {
